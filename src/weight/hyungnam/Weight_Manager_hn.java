...
public class Weight_Manager_hn {
	...
	public static void main(String[] args) {
		Weight_Manager_hn wm = new Weight_Manager_hn();
		int choiceInt;
		...
		while (true) {
			try {
				...
				wm.keyboard.nextLine(); // ?다음 입력을 위해 날림 질문
				System.out.println();

				// + 정수 1~5까지 입력 범위 제한하기 위해서 MenuChoiceException을 발생시킨다.
				if (choiceInt < 1 || choiceInt > 5)
					throw new MenuChoiceException(choiceInt);
			...
			// + 사용자 정의 Exception으로 주어진 메뉴의 범위를 벗어나면 해당 블럭에 잡힌다.
			...
			// + 입출력 관련한 Exception 발생시 해당 블럭에 잡힌다.
			catch (IOException e) {
				System.out.println(e + "오류발생");

			}
			// + Scanner로 값 입력 받을시 nextInt() 함수를 이용해 받기 때문에 정수가 아닌 다른 문자가 왔을때 해당 블럭에 잡힌다.
			catch (Exception e) {
	...
	public void inputWeight() throws IOException {
		...
		// + 지정 경로 하위(C://WeightMgData/)에 [입력 이름].txt 파일 객체를 생성한다.
		...
		if (!f.exists()) {
			...
			// + 기본 경로(fd = 멤버 필드 = C://WeightMgData/)가 존재하지 않으면 생성한다.
			if (!fd.exists())
				fd.mkdir(); // C://WeightMgData/폴더 생성
		...
		// +C://WeightMgData/ [입력 이름].txt 이 이미 존재 할 때
		else {
	...
	public boolean fileCheck() throws IOException {
		...
		// + 기본 경로(fd = 멤버 필드 = C://WeightMgData/)에 하위에 [입력 이름].txt 파일 경로 생성한다.
		f = new File(fd.getAbsolutePath() + "/" + this.str_name + ".txt"); // 입력받은이름으로 파일경로 생성

		// + C://WeightMgData/[입력 이름].txt이 존재할 때
		if (f.exists()) { // 파일이 존재하면
			fr = new FileReader(f); // 파일 읽을 준비
			br = new BufferedReader(fr); // + Buffer에 파일을 담는다.
...
// + Java 기본 Exception이 아닌 사용자가 정의한 Exception을 담는 클래스
